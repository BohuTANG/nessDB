                          ...
                          struct msg *spk)
{
	...
	parent->u.n.parts[pidx].buffer = nmb_new();
                 ...
                 struct msg **split_key)
{
	...
	leafa = leaf;
	mb = leafa->u.l.buffer;
	lmb_split(mb, &mba, &mbb, split_key);
                    ...
                    struct msg **split_key)
{
	...
	nodea->u.n.parts[pivots_in_a].buffer = nmb_new();
                 ...
                 struct node *old_root)
{
	...
	new_root->u.n.parts[0].buffer = nmb_new();

	new_root->u.n.parts[1].child_nid = b->nid;
	new_root->u.n.parts[1].buffer = nmb_new();
	msgfree(split_key);
