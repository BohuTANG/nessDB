...
int msgpack_pack_nmbiter(struct msgpack *pk, struct nmb_iter *iter)
{
	MSN msn = iter->msn;

	msn = ((msn << 8) | iter->type);
	msgpack_pack_uint64(pk, msn);
	msgpack_pack_uint64(pk, iter->xidpair.child_xid);
	msgpack_pack_uint64(pk, iter->xidpair.parent_xid);
	msgpack_pack_uint32(pk, iter->key.size);
	msgpack_pack_nstr(pk, iter->key.data, iter->key.size);

	if (iter->type != MSG_DELETE) {
		msgpack_pack_uint32(pk, iter->val.size);
		msgpack_pack_nstr(pk, iter->val.data, iter->val.size);
	}

	return 1;
}

int msgpack_unpack_nmbiter(struct msgpack *pk, struct nmb_iter *iter)
{
	if (!msgpack_unpack_uint64(pk, &iter->msn)) goto ERR;
	iter->type = (iter->msn & 0xff);
	iter->msn = (iter->msn >> 8);
	if (!msgpack_unpack_uint64(pk, &iter->xidpair.child_xid)) goto ERR;
	if (!msgpack_unpack_uint64(pk, &iter->xidpair.parent_xid)) goto ERR;
	if (!msgpack_unpack_uint32(pk, &iter->key.size)) goto ERR;
	if (!msgpack_unpack_nstr(pk, iter->key.size, (char**)&iter->key.data)) goto ERR;
	if (iter->type != MSG_DELETE) {
		if (!msgpack_unpack_uint32(pk, &iter->val.size)) goto ERR;
		if (!msgpack_unpack_nstr(pk, iter->val.size, (char**)&iter->val.data)) goto ERR;
	}
...
int msgpack_pack_lmbiter(struct msgpack *pk, struct lmb_iter *iter)
{
	MSN msn = iter->msn;
...
int msgpack_unpack_lmbiter(struct msgpack *pk, struct lmb_iter *iter)
{
	if (!msgpack_unpack_uint64(pk, &iter->msn)) goto ERR;
	iter->type = (iter->msn & 0xff);
	iter->msn = (iter->msn >> 8);
